...
export const usePeople = () => useSupabaseQuery<Person>('kisiler', { realtime: false });
...
export const useDashboardData = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        setError('');
        try {
            const [peopleResponse, projectsResponse, applicationsResponse, donationsResponse] = await Promise.all([
                supabase.from('kisiler').select('*'),
                supabase.from('projeler').select('*'),
                supabase.from('yardim_basvurulari').select('*'),
                supabase.from('bagislar').select('*'),
            ]);
