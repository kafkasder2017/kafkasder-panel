...
const transformPersonToDB = (person: Partial<Person>): any => {
    ...
    const allowedDbFields = [
        'email', 'adres', 'city', 'district', 'neighborhood',
    ...
    // Handle field mapping for Turkish column names
    if (person.address) dbPerson.adres = person.address;
...
export const getPeople = async (): Promise<Person[]> => {
    const { data, error } = await supabase.from('kisiler').select('*').order('created_at', { ascending: false });
...
export const getPersonById = async (id: string): Promise<Person> => {
    const { data, error } = await supabase.from('kisiler').select('*').eq('id', id).single();
...
export const createPerson = async (person: Omit<Person, 'id'>): Promise<Person> => {
    ...
    const { data, error } = await supabase.from('kisiler').insert([dbPerson]).select().single();
...
export const updatePerson = async (id: string, person: Partial<Person>): Promise<Person> => {
    const dbPerson = transformPersonToDB(person);
    const { data, error } = await supabase.from('kisiler').update(dbPerson).eq('id', id).select().single();
...
// Projeler
export const getProjeler = (): Promise<Proje[]> => getAllSafe<Proje>('projeler');
...
// Bağışlar
export const getBagislar = (): Promise<Bagis[]> => getAllSafe<Bagis>('bagislar');
...
// Yardım Başvuruları
export const getYardimBasvurulari = (): Promise<YardimBasvurusu[]> => getAllSafe<YardimBasvurusu>('yardim_basvurulari');
